    ...
    if (params.date === todayStr || params.date === yesterdayStr) {
        ...
        liveRef.on('value', (snapshot) => {
            ...
            if (incomingData) {
                ...
                // --- 4. DIRECT FIREBASE OVERWRITE (Trust the Python Engine) ---
                ...
                if (needsFullRender) {
                    liveGamesArray.forEach(liveGame => {
                        ...
                        if (index !== -1) {
                            // The Python engine already formatted everything perfectly. 
                            // Just protect the aggregate goals.
                            const old_first_leg = ALL_GAMES_DATA[index].first_leg_goals;
                            ALL_GAMES_DATA[index] = liveGame; 
                            ALL_GAMES_DATA[index].first_leg_goals = liveGame.first_leg_goals || old_first_leg;
                        } else {
...
function syncLiveDOM(liveGamesArray) {
    liveGamesArray.forEach(match => {
        ...
        if (oldMatchIndex !== -1) {
            ...
            // The Python engine already formatted everything perfectly. 
            // Just protect the aggregate goals.
            const old_first_leg = oldMatch.first_leg_goals;
            ALL_GAMES_DATA[oldMatchIndex] = match; 
            ALL_GAMES_DATA[oldMatchIndex].first_leg_goals = match.first_leg_goals || old_first_leg;
        } else {
