    ...
    for m in matches_to_process:
        ...
        for side in ['home', 'away']:
            ...
            if t_slug in team_state:
                ...
                # 🚨 FAILSAFE: If the API has the lineup, but the local HTML file is missing it, force a rebuild
                has_api_lineup = bool((m.get(side + 'Lineup') or {}).get('startXI'))
                if has_api_lineup and not needs_update:
                    ...
                    if not os.path.exists(index_file):
                        ...
                        try:
                            with open(index_file, 'r', encoding='utf-8') as f:
                                # This string is generated by generate_pitch_html() when no lineup exists
                                if "Awaiting Live Lineup Data" in f.read():
                                    needs_update = True
                        except Exception:
                            needs_update = True
