...
<script>
    ...
    function updateMatchDOM(domId, data) {
        ...
        if (data.events && data.events.length > 0) {
            ...
            if (eventsBlock) {
                let timelineHtml = `<div class="w-100 px-3 pt-3 pb-2 mt-2 border-top" style="background-color: #f8f9fa; border-radius: 0 0 8px 8px; max-height: 200px; overflow-y: auto;">`;
                const reversedEvents = [...data.events].reverse();
                reversedEvents.forEach(ev => {
                    let icon = '🟨', colorClass = 'text-warning';
                    if (ev.type === 'Goal') { icon = '⚽'; colorClass = 'text-success'; }
                    else if (ev.type === 'Red Card') { icon = '🟥'; colorClass = 'text-danger'; }
                    else if (ev.type === 'subst') { icon = '🔄'; colorClass = 'text-primary'; }

                    if (ev.type === 'subst') {
                        timelineHtml += `
                        <div class="d-flex align-items-start mb-2 border-bottom pb-2">
                            <div class="text-secondary fw-bold pe-2" style="width: 40px; text-align: right; font-size: 0.75rem;">${ev.time}'</div>
                            <div style="width: 25px; text-align: center; font-size: 0.85rem;" class="me-2">${icon}</div>
                            <div class="text-truncate" style="font-size: 0.75rem; line-height: 1.2;">
                                <span class="text-dark fw-bold">${ev.player}</span> IN<br>
                                <span class="text-muted">(${ev.player_out} OUT)</span>
                            </div>
                        </div>`;
                    } else {
                        const astHtml = ev.assist ? `<br><span class="text-muted fw-normal">👟 ${ev.assist}</span>` : '';
                        timelineHtml += `
                        <div class="d-flex align-items-start mb-2 border-bottom pb-2">
                            <div class="text-secondary fw-bold pe-2" style="width: 40px; text-align: right; font-size: 0.75rem;">${ev.time}'</div>
                            <div style="width: 25px; text-align: center; font-size: 0.85rem;" class="me-2 ${colorClass}">${icon}</div>
                            <div class="text-truncate" style="font-size: 0.75rem; line-height: 1.2;">
                                <span class="text-dark fw-bold">${ev.player}</span>${astHtml}
                            </div>
                        </div>`;
                    }
                });
                timelineHtml += `</div>`;
                eventsBlock.innerHTML = timelineHtml;
            }
        }
    }
        ...
        if (data.events && data.events.length > 0) {
            ...
            if (eventsBlock) {
                ...
                reversedEvents.forEach(ev => {
                    ...
                    if (ev.type === 'subst') {
                        timelineHtml += `<div class="d-flex align-items-start mb-2 border-bottom pb-2"><div class="text-secondary fw-bold pe-2" style="width: 40px; text-align: right; font-size: 0.75rem;">${ev.time}'</div><div style="width: 25px; text-align: center; font-size: 0.85rem;" class="me-2">${icon}</div><div class="text-truncate" style="font-size: 0.75rem; line-height: 1.2;"><span class="text-dark fw-bold">${ev.player}</span> IN<br><span class="text-muted">(${ev.player_out} OUT)</span></div></div>`;
                    } else {
                        const astHtml = ev.assist ? `<br><span class="text-muted fw-normal">👟 ${ev.assist}</span>` : '';
                        timelineHtml += `<div class="d-flex align-items-start mb-2 border-bottom pb-2"><div class="text-secondary fw-bold pe-2" style="width: 40px; text-align: right; font-size: 0.75rem;">${ev.time}'</div><div style="width: 25px; text-align: center; font-size: 0.85rem;" class="me-2 ${colorClass}">${icon}</div><div class="text-truncate" style="font-size: 0.75rem; line-height: 1.2;"><span class="text-dark fw-bold">${ev.player}</span>${astHtml}</div></div>`;
                    }
                });
                timelineHtml += `</div>`;
                eventsBlock.innerHTML = timelineHtml;
