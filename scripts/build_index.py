        ...
        try:
            ...
            if state == 'post' and old_html:
                ...
                if saved_block:
                    ...
                    badge_list = ['>FT</span>', '>AET</span>', '>PEN</span>', '>PST</span>', '>CANC</span>', '>ABD</span>']
                    matched_badge = next((b for b in badge_list if b in card_content), None)
                    
                    if matched_badge:
                        # Extract the actual text from the badge (e.g., 'PST', 'FT', 'CANC')
                        actual_status = matched_badge.replace('>', '').replace('</span>', '')
                        matches.append({
                            "fixture": {"id": event_id, "date": event.get('date', ''), "status": {"short": actual_status}},
